...
## Code-along 2: File operations mv, cp and rm

0. If you are not there already, go to the `Images` folder under `Exercises`. 

1. There is a file named `.txt` in the `Images` folder. Those files are usually text files. It should not be there since it is not an image. Move the text file from the `Images` folder to the `Documents` folder. Confirm that it is in the right place and that it does not exist inside the `Images` folder anymore.

2. Make a copy of the `mytextfile.txt` file (should now be in `Documents`) and name the copy whatever you'd like. 

3. Move to the `Exercises` directory and make a new folder here called `TEMP`. Now move the copied text file you created in point 2 to the `TEMP` folder. Confirm that it is in the right location.
