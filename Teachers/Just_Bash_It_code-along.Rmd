...
Go to where you have placed the unzipped git repo. If it is on your Desktop you would do:
...
Then navigate to where you have placed the unzipped git repo. If it is on your Desktop you would do:
...
1. List the contents of the workshop repo. You should see something like this:

![](Figures/list_repo.PNG)

2. What happens if you add the flag `-l` (that's a letter l) to the `ls` command?

```{bash, engine="sh", eval=FALSE}
ls -l
```

We are going to work in the `Examples` folder. 

3. Change directory to the `Examples` folder and list the contents.

4. List the contents of the `Documents` folder?

5. Change directory into the `Documents` folder and check what your current working directory is. It should say this (provided your Just Bash t folder is on your Desktop):

```{bash, engine="sh", eval=FALSE}
~/Desktop/Just-Bash-It/Examples/Documents
```
...
8. Then go back the `Images` folder (Hint: This folder should have been listed under point 3). Can you do it using the `../` syntax?

9. What is the relative path to the text file in the `Courses` folder from where you are right now? You can check your answer by using `ls [relative path to file]` and seeing whether it shows the file.
