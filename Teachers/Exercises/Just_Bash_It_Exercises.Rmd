...
2. Now, add the flag `-l` (that's the small letter L) to the `ls` command. What changes?
    ...
    Use the help of ls `ls --help` (git bash) or `man ls` (on mac/linux) as well as what you learned in the lecture to answer the following questions:

  * Which listed items are files and which are directories?
  * Which file in `Just-Bash-It` is the largest?
  * Are there any hidden files (that you normally don't see with a simple `ls`)?   
  
    If you cannot see the relevant infos like size, use the `-l` option in combination with other options found in the help (or by googling). 
...
7. What is the full path of the file inside the `Courses` folder? You can test your answer with 
...
Inside the `projects` directory, create a directory for this course. You could name it i.e. **Just_Bash_It, Intro_to_command_line, First_Project, etc.**. Then inside that directory, create all the sub-directories shown on **slide 43** in the slideshow. 
...
Go to your directory `Scratch` where you should have a copy of the unzipped `GCF_genomicAnnotation.gff.gz` file named `Annotation.gff`, which you made in exercise 3.5 above. We will now do some data wrangling operations on the file. You can find all necessary commands on **slide 63** in the presentation (if they aren't named in the question). 
...
3. For further analysis, we want to cut out only the columns that have the information on the *Region*, *Start* and *End* of the genetic element. You will do this by using `cut` on `Annotation_tmp.gff` and redirecting the output into a new file called `GenomicRegion.tmp`.   
**HINT1:** First, identify the position (1,2,3,ect) of the columns we want.   
**HINT2:** A challenge you might encounter is that the annotation file is separated by the *tab* delimiter, which in unix is written as: `\t`. For some systems, `cut` without a specified delimiter will work fine. *If* we need to specify the delimiter as `\t` (instead of say space) we need to put a `$` in front of it like so: `-d$'\t'` (there are no spaces here!). Don't ask, it's a long story... Check that your output file looks correct.   

Next, we would like to extract the gene names. They are in field/column 7 in the `Annotation_tmp.gff`, but field 7 contains itself several entries like ID, gene name, locus tags etc. separated by a `;`. I.e. field 7 contains sub-fields delimited by `;`.   

4. Extract the gene name by using `cut` twice. First, cut out column 7 from `Annotation_tmp.gff` and name that temporary file `col7.tmp`. Inspect `col7.tmp` for example with `head` and count which of the subfields you want to extract. We want the sub-field starting with `gene=`. Some lines in the gff file do not have the `gene=` field because they are other genomic features. But since this is just an exercise, we will be content with cutting based on the field's position. When you have found it, cut `col7.tmp` and put the result into `gene_names.txt`.   
...
You should now have one file `GenomicRegion.tmp` containing only field 3, 4 & 5 from exercise 5.3 and one `gene_names_clean.txt` containing gene names from exercise 5.5.   

1. Paste these two files together into a single file using the command `paste`. Name this file `Annotation_Gene.gff`. The new file should be tab separated, just like the `GenomicRegion.tmp` file. To do so, use the delimiter flag to specify what kind of delimiter should be used to paste the fields together (tab is `\t`). 
...
Your new file `Annotation_Gene_Len.gff` does not have a header line explaining what the different columns are. 

4. Using an editor, i.e nano, make a new file called `header.gff` with these column names:   
...
6. Finally, let's **redo points 2-5 from Exercise 5 and point 1 from Exercise 6**, starting from the `Annotation.gff` file, BUT this time using pipes to reduce the number of intermediate files. You should end up with a file containing these 4 columns: `Region, Start, End and gene name (only)`. Use your `history` if you can't remember the commands you used.   
    ...
    There are many ways of achieving this. For those who like a challenge, it is possible to get to the final output file in two command lines (i.e. only one intermediate file) by using `-` to read input from stdin (instead of specifying the name of the input file).   
...
**HINT:** Remember you can use commands like `man` and the option `-help` to help you understand what is going on. 
...
**HINT:** Make sure you spell the application name correctly, i.e. capital letters, spaces, etc.


3. If you have a Mac or linux OS, you can now kill the application you just opened by using the command line. You will have to combine the commands `ps aux` and `grep` to get the `process id (pid)` for the application. What does the `-9` mean? On gitbash this is not possible, so just proceed to the next point.
...
In the second part of the exercise we will now install and run a software thought the command line. The software is [fastqc](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/). It is made for quality control of DNA/RNA sequencing data. The software produces a report with different statistics of read quality and bias checks.
...
On Ubuntu we generally use `apt-get` to install software. 

First, update apt database (you might need sudo): 
```{bash, engine="sh", eval=FALSE}
apt-get update
```

Install `fastqc` (you might need sudo):
```{bash, engine="sh", eval=FALSE}
apt-get -y install fastqc
```

If you have linux distribution that doesn't have `apt-get` either google how to install things or download the java app of fastqc (see Windows) and run it with java.

&nbsp;

**Windows Users:**

On windows, we will use java to execute the fastqc executable (which is written in java).
Check whether you have java:

```{bash, engine="sh", eval=FALSE}
java -version
```

If you do not have java, download and install it:
https://www.java.com/en/download/windows_manual.jsp
I have used the Windows Offline 64-bit. 

Next, download fastqc from here: https://www.bioinformatics.babraham.ac.uk/projects/fastqc/ . Unzip the folder, either with the command line or by right clicking on it and put the content where you want them to be. 

Now you can navigate to where the executable is and see whether you are allowed to run it. For me it looks like this:

![](Figures/install_fastqc.PNG)

You can also run fastqc from any other directory by providing the full path to the executable (which you now have because you just checked it):

![](Figures/run_fastqc.PNG)
...
## Exercise 10: Computing on HPCs 

For exercise 10, you will log in to our small teaching server. Emphasis on small, it only has 8 cores so some waiting time may occur for executing commands. 

Part 1: Discovering the server

1. Log in to the server. You should have been given a number. This number + `stud` is the account you should use. All student account have the same password, `pass1234`. We log in with ssh:

    ```{bash, eval = F}
    ssh stud[number]@157.180.86.52
    ```

    You may be asked whether you want to continue to this server which is not in your known hosts. Select Yes. Provide the password. 

    You will know you have successfully logged in if your prompt has changed to `stud[number]@bash-teaching`. 
    
    ![](Figures/ssh_in.png)

2. Take a moment to look around and locate where folders are. You have a home directory, which is the place where you landed directly after logging in. Check its path (`pwd`). Locate the project directory (`/project`) and check which directories it contains. 

3. Use `module avail` to display the software available in environment modules. 

4. Invoke the aligner tool bwa with:
...
5. Load the bwa module `module load bwa` and invoke bwa again.

6. Confirm which modules are loaded with `module list`.

7. Unload the module with `module unload`.

8. Check the server load with `htop` (press q to quit).

&nbsp;

Part 2: Running a small bioinformatics job

We will now submit a job to slurm. We will map reads of *Arabidopsis thaliana* to the reference genome, then sort the alignment file and get a report on mapping success with `flagstat`. We provide the script for you so there will not be an issue with the commands.

9. First, change into the project directory for *Arabidopsis thaliana* (`a_thal`). You should see a `results` directory. Inside there, create a directory with your own name. You will put your results into that directory so you don't overwrite each others results.


10. Locate the job script in the folder `/home/boss/scripts/`. Copy the script to your OWN home folder. View it to see the different steps. Then using nano or a text editor of your choice, open the job script and change the name of the job to YOUR name so it will be easier to track. Also, everywhere it says `[YOU]` change that to the name of your folder (that you just created).  

10. When you are ready submit the script to the queue with 

    ```{bash, eval = F}
    sbatch job.sh
    ```

11. Use the `squeue` command to check whether your job is running. If the state (`ST`) is `PD` the resource allocation is pending. If it is `R` the job is running and if it is `ST` the job has stopped/finished. `squeue` is not interactive, so you can check several times to to see if your status has changed after a while. Have another look at server load with `htop`. You will probably see many of the CPUs busy if jobs are running. 

12. After your job has finished you will have a file named `slurm-[jobid].out` in the folder from which you started the job. You can have a look at it, it shows information from the mapping. Go to your folder in the project results and confirm whether all results were made. You should have 3 files:

    ```{bash, eval = F}
    ls
    aln-pe.sam  flagstat.txt  sorted.bam
    ```

    If not, perhaps something has gone wrong during the executing of the job. There should be info about it in the `slurm` file. 
