...
3. If you have a Mac of linux OS, you can now kill the application you just opened by using the command line. You will have to combine the commands `ps aux` and `grep` to get the `process id (pid)` for the application. What does the `-9` mean? On gitbash this is not possible, so just proceed to the next point.
...
You can also run fastqc from any other directory by providing the fullpath to the executable (which you now have because you just checked it):
