...
As you will have noticed `field 7` in the annotation file has multiple information per entry (ids, gene name, locus, tags etc.) separated by a `;`. We refer to such separating characters as `delimiters`.   
You are only interested in one information from `field 7`, and that is the **gene name**. To extract this information we will use two commands, first `cut` to cut the field out and then `grep` to grep the pattern we want in the sub-field (between two `;`).
...
8. Let's have a look at the content of your final file.
  * Are there any gene sequences with length 0 in the annotation file? - Try the command `sort` with flags  `-f 5`
